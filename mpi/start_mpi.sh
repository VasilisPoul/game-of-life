...
python3 scripts/block.py $rows generations/row/input.txt
...
ID=$(qsub -l select=$select:ncpus=$ncpus:mpiprocs=$mpiprocs -v proc=$mpiprocs,rows=$rows,cols=$cols mpiPBSscript.sh | sed -e s/"\..*"//)
