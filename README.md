...
Ο ρόλος των νημάτων είναι να παραλληλοποιηθεί όσο το δυνατόν περίσσότερο το κάθε βήμα της κεντρικής επανάλληψης. Για το λόγο αυτό, στο αρχικό νήμα (**master**) πριν ξεκινήσει η κεντρική επανάληψη, δημιουργούνται νήματα όπου το κάθε ένα από αυτά θα τρέξει όλα τα βήματα με σκοπό να παραλληλοποιηθούν οι εσωτερικές λούπες που κάνουν τα calculations και τα βήματα τους να μοιραστούν σε κάθε thread. 
...
| |1 threads / block|2 threads / block|4 threads / block| 8 threads / block| 16 threads / block|32 threads / block| 64 threads / block |  
|--|--|--|--|--|--|--|--|  
| **320x320** | 0.39 | 0.12 | 0.07 | **0.06** | 0.07 | 0.07 | 0.08 |  
| **640x640** | 1.30 | 0.39 | 0.19 | 0.15 | 0.19 | 0.20 | **0.09** |  
| **1280x1280** | 3.89 | 1.24 | 0.61 | 0.48 | 0.56 | 0.62 | **0.29** |  
| **2560x2560** | 14.29 | 4.30 | 1.99 | 1.51 | 1.79 | 1.94 | **0.83** |  
| **5120x5120** | 59.11 | 17.71 | 8.11 | 5.95 | 6.70 | 7.47 | **3.10** |  
  
Παρατηρούμε, πως με 1 thread/block, οι χρόνοι εκτέλεσης σε όλα τα μεγέθη του προβλήματος είναι πολύ αργοί. Αυτό οφείλεται στο οτι  κάθε thread, παίρνει τους γειτονές του από τη global memory, με αποτέλεσμα να γίνεται access στη global memory πολλές αυξάνοντας έτσι τις καθυστερήσεις.  
  
Επίσης, όσο αυξάνεται το μέγεθος του προβλήματος, τόσο πιο πολύ αυξάνεται η επιτάχυνση, όσο αυξάνουμε τα threads.  
  
Δοκιμάσαμε να το τρέξουμε και με 128/256/512/1024 threads/block χωρίς επιτυχία, αφού λαμβάναμε λάθος αποτελέσματα, ενώ εμφανιζόταν το παρακάτω error:   
